...
from ladder_decompose import iter_training_pairs
from ladder_mcts import BuildState, mcts_search, weighted_rollout
from ladder_search import program_str
from ladder_sim import And, Contact, Or
...
def make_prior_fn(w, temp: float = 1.0):
  ...
  def prior(state, acts):
    return _softmax(np_forward(w, featurize_pairs(state, acts)), temp).tolist()
...
def main():
  ...
  print(f'featurize: state {STATE_DIM} + action {ACTION_DIM} = {FEAT_DIM} dim')
  if holdout:
