...
from ladder_benchmark import METHODS, make_tasks, random_program_ext
from ladder_mcts import BuildState
from ladder_search import (
    accuracy, evaluate, find_coil_conflicts, program_str,
)
from ladder_simplify import shrink_program, simplify_program

tasks = make_tasks()

for t in tasks:
    masked = sum(
        1 for sc in t.spec.scenarios for w in sc.expected
        for v in w.values() if v is None
    )
