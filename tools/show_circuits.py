...
from ladder.simplify import polish_program, simplify_program
...
if __name__ == '__main__':
    ...
    for name in names:
        ...
        stage(
            f'[레퍼런스 해]  (사람이 작성, 크기 {program_size(t.reference)})',
            t.reference,
        )
        found, acc, prog = run_gp(t, BUDGET, SEED)
        if found:
            stage(
                f'[GP 발견 해]  ({found:,}회 평가, 크기 {program_size(prog)})',
                prog,
            )
        else:
        ...
        stage(
            f'[단순화 후]  (동작 보존, 크기 {program_size(prog)} → '
            f'{program_size(simple)})',
            simple,
        )
        ...
        stage(
            f'[스펙 축소 후]  (스펙만 보존, 크기 '
            f'{program_size(simple)} → {program_size(final)})',
            final,
        )
        print()
