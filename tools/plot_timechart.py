...
def draw_wave(ax, base, vals, color):
    ...
    for t, v in enumerate(vals):
        if v is None:
            ...
            continue
        y = base + v * HIGH
        ax.plot([t, t + 1], [y, y], color=color, lw=2.2, solid_capstyle='butt')
...
def plot_scenario(task, idx):
    ...
    rows = [(d, [r[d] for r in in_rows], IN_COLOR) for d in task.spec.inputs]
    rows += [
        (d, [w.get(d) for w in sc.expected], OUT_COLOR)
    ...
    ax.set_title(
        f"{task.name} — 시나리오 {idx}  (회색 = don't-care)",
        fontsize=11,
        loc='left',
    )
    fig.tight_layout()
