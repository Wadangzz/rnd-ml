...
from ladder_benchmark import GenCfg, Task, derive_spec, mask_transition_scans
from ladder_search import evaluate
from ladder_sim import And, Coil, Contact, Or, Program, Rung, Timer
...
def make_chain_task(
  ...
  chain = list(ys)
  spec.invariants = [
    lambda out, chain=chain: sum(out[y] for y in chain) <= 1
  ]
...
def make_chain_curriculum(
  ...
  while len(tasks) < n_variants:
    ...
    try:
      tasks.append(
        make_chain_task(K, xs, ys, x_pool, y_pool, f'chain{K}_v{len(tasks)}')
      )
...
def make_timer_chain_reference(K: int, xs: list, ys: list) -> Program:
  ...
  for k in range(K, 1, -1):
    ...
    clear = (
      Contact(xs[K], 'NC') if k == K else Contact(ys[k], 'NC')
    )
    rungs.append(
      Rung(
        Coil(ys[k - 1]),
