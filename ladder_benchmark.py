...
from ladder_gp import gp_search
from ladder_mcts import (
  BuildState,
  mcts_search,
  weighted_rollout,
)
...
def assert_spec_not_degenerate(spec: Spec):
  ...
  for logic, label in [
    ...
    assert acc < 1.0, f"don't-care 과다: {label} 회로가 통과 (acc={acc:.3f})"
  ...
  ref = Program(
    [
      Rung(
        ...
        And(
          [
            Or([And([Contact('X1'), Contact('Y0')]), Contact('Y1')]),
            Contact('X2', 'NC'),
          ]
        ),
      ),
  ...
  ref = Program(
    [
      Rung(
        ...
        And(
          [
            Or([And([Contact('X2'), Contact('Y1')]), Contact('Y2')]),
            Contact('X3', 'NC'),
          ]
        ),
      ),
...
def random_logic_ext(devices, depth, rng, cfg: GenCfg, name_counter):
  ...
  if cfg.timer_presets and rng.random() < cfg.wrap_p:
    node = Timer(f'T{next(name_counter)}', rng.choice(cfg.timer_presets), node)
...
def random_program_ext(spec: Spec, rng, cfg: GenCfg) -> Program:
  ...
  for _ in range(rng.randint(1, cfg.max_rungs)):
    ...
    rungs.append(
      Rung(
        Coil(dev, op),
        random_logic_ext(contact_pool, cfg.max_depth, rng, cfg, name_counter),
      )
    )
...
if __name__ == '__main__':
  ...
  for t in tasks:
    for method, runner in METHODS:
      for seed in SEEDS:
        ...
        print(f'  {t.name:<12} {method:<7} seed{seed}: {stat}', flush=True)
  ...
  print('=' * width)
  print(
    f'{"과제":<13}' + ''.join(f' | {m + " (3시드)":>26}' for m, _ in METHODS)
  )
  print('-' * width)
  by = {}
  ...
  for t in tasks:
    ...
    def fmt(method):
      ...
      for found, acc in by[(t.name, method)]:
        cells.append(f'{found:,}' if found else f'x(.{int(acc * 1000):03d})')
